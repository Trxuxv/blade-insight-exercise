<template>
  <div class="w-full h-screen bg-gradient-to-b from-gray-200 to-gray-600">
    ...
    <div
      v-show="!loading"
      class="w-5/6 bg-white h-4/6 mx-auto rounded-lg mt-8 overflow-hidden"
    >
      <div
        class="w-4/6 float-left h-full vector"
        :style="{
      ...
      <div class="w-2/6 float-right h-full bg-gray-600 rounded-r-lg px-8">
        ...
        <div class="w-full h-16 mt-6 grid grid-cols-4 rounded-lg">
          ...
          <div
            class="text-center h-4/5 bg-gray-200 px-7 py-1.5 text-sm font-semibold rounded-r-lg"
          >
    ...
    <div
      v-show="loading"
      class="w-5/6 bg-white h-4/6 rounded-lg flex items-center justify-center mx-auto mt-10"
    >
      <svg
        role="status"
...
export default Vue.extend({
  ...
  computed: {
    country() {
      return this.countries.find((x: CountryModel) =>
